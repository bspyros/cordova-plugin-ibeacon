...
public class LocationManager extends CordovaPlugin implements BeaconConsumer {
  ...
  private static final String FOREGROUND_BETWEEN_SCAN_PERIOD_NAME =
    "com.unarin.cordova.beacon.android.altbeacon.ForegroundBetweenScanPeriod";
  private static final String FOREGROUND_SCAN_PERIOD_NAME =
    "com.unarin.cordova.beacon.android.altbeacon.ForegroundScanPeriod";
  private static final int DEFAULT_FOREGROUND_BETWEEN_SCAN_PERIOD = 0;
  private static final String SAMPLE_EXPIRATION_MILLISECOND =
    "com.unarin.cordova.beacon.android.altbeacon.SampleExpirationMilliseconds";
  private static final int DEFAULT_SAMPLE_EXPIRATION_MILLISECOND = 20000;
  private static final String ENABLE_ARMA_FILTER_NAME =
    "com.unarin.cordova.beacon.android.altbeacon.EnableArmaFilter";
  private static final boolean DEFAULT_ENABLE_ARMA_FILTER = false;
  private static final String REQUEST_BT_PERMISSION_NAME =
    "com.unarin.cordova.beacon.android.altbeacon.RequestBtPermission";
  ...
  public void initialize(CordovaInterface cordova, CordovaWebView webView) {
    ...
    final int foregroundBetweenScanPeriod =
      this.preferences.getInteger(
          FOREGROUND_BETWEEN_SCAN_PERIOD_NAME,
          DEFAULT_FOREGROUND_BETWEEN_SCAN_PERIOD
        );

    final int foregroundScanPeriod =
      this.preferences.getInteger(
          FOREGROUND_SCAN_PERIOD_NAME,
          DEFAULT_FOREGROUND_SCAN_PERIOD
        );

    Log.i(
      TAG,
      "Determined config value FOREGROUND_SCAN_PERIOD: " +
    ...
    final int sampleExpirationMilliseconds =
      this.preferences.getInteger(
          SAMPLE_EXPIRATION_MILLISECOND,
          DEFAULT_SAMPLE_EXPIRATION_MILLISECOND
        );

    Log.i(
      TAG,
      "Determined config value SAMPLE_EXPIRATION_MILLISECOND: " +
      String.valueOf(sampleExpirationMilliseconds)
    );

    final boolean enableArmaFilter =
      this.preferences.getBoolean(
          ENABLE_ARMA_FILTER_NAME,
          DEFAULT_ENABLE_ARMA_FILTER
        );
    ...
    final boolean requestPermission =
      this.preferences.getBoolean(
          REQUEST_BT_PERMISSION_NAME,
          DEFAULT_REQUEST_BT_PERMISSION
        );
  ...
  private BeaconTransmitter createOrGetBeaconTransmitter() {
    if (this.beaconTransmitter == null) {
      ...
      this.beaconTransmitter =
        new BeaconTransmitter(getApplicationContext(), beaconParser);
    }
  ...
  private void tryToRequestMarshmallowLocationPermission() {
    if (Build.VERSION.SDK_INT < BUILD_VERSION_CODES_M) {
      Log.i(
        TAG,
        "tryToRequestMarshmallowLocationPermission() skipping because API code is " +
        "below criteria: " +
    ...
    if (checkSelfPermissionMethod == null) {
      Log.e(
        TAG,
        "Could not obtain the method Activity.checkSelfPermission method. Will " +
    ...
    try {
      ...
      Log.i(
        TAG,
        "Permission check result for ACCESS_COARSE_LOCATION: " +
      ...
      if (permissionCheckResult == PackageManager.PERMISSION_GRANTED) {
        Log.i(
          TAG,
          "Permission for ACCESS_COARSE_LOCATION has already been granted."
        );
        return;
      }
      ...
      if (requestPermissionsMethod == null) {
        Log.e(
          TAG,
          "Could not obtain the method Activity.requestPermissions. Will " +
        ...
        new DialogInterface.OnDismissListener() {
          ...
          public void onDismiss(final DialogInterface dialog) {
            try {
              ...
              Log.e(
                TAG,
                "IllegalAccessException while requesting permission for " +
              ...
              Log.e(
                TAG,
                "InvocationTargetException while requesting permission for " +
                "ACCESS_COARSE_LOCATION:",
                e
              );
            }
      ...
      Log.w(
        TAG,
        "IllegalAccessException while checking for ACCESS_COARSE_LOCATION:",
        e
      );
    } catch (final InvocationTargetException e) {
      Log.w(
        TAG,
        "InvocationTargetException while checking for ACCESS_COARSE_LOCATION:",
        e
      );
    }
  }
  ...
  public void requestBluetoothPermissions() {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
      ...
      if (!hasPermissions(cordova.getContext(), PERMISSIONS)) {
  ...
  private void initBluetoothListener() {
    ...
    if (!hasBlueToothPermission()) {
      debugWarn(
        "Cannot listen to Bluetooth service when BLUETOOTH permission is not added"
      );
      return;
    }
    ...
    try {
      ...
      debugWarn(
        "Unexpected exception checking for Bluetooth service: " + e.getMessage()
      );
      return;
    }
      ...
      new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
          ...
          if (action.equals(BluetoothAdapter.ACTION_STATE_CHANGED)) {
            ...
            debugLog(
              "Bluetooth Service state changed from " +
            ...
            switch (state) {
              case BluetoothAdapter.ERROR:
                beaconServiceNotifier.didChangeAuthorizationStatus(
              ...
              case BluetoothAdapter.STATE_TURNING_OFF:
                if (
                  oldState == BluetoothAdapter.STATE_ON
                ) beaconServiceNotifier.didChangeAuthorizationStatus(
  ...
  private void checkEventQueue() {
    if (threadPoolExecutor != null && queue != null) return;
  ...
  private void createMonitorCallbacks(final CallbackContext callbackContext) {
    ...
    iBeaconManager.setMonitorNotifier(
      new MonitorNotifier() {
        ...
        private void dispatchMonitorState(
            ...
            new Runnable() {
              public void run() {
                try {
                  ...
                  Log.e(
                    TAG,
                    "'monitoringDidFailForRegion' exception " + e.getCause()
                  );
    ...
    iBeaconManager.setRangeNotifier(
      new RangeNotifier() {
        ...
        public void didRangeBeaconsInRegion(
          final Collection<Beacon> iBeacons,
          final Region region
        ) {
          threadPoolExecutor.execute(
            new Runnable() {
              public void run() {
                try {
                  ...
                  Log.e(
                    TAG,
                    "'rangingBeaconsDidFailForRegion' exception " + e.getCause()
                  );
                  beaconServiceNotifier.rangingBeaconsDidFailForRegion(
                    region,
                    e
                  );
                }
  ...
  private void createManagerCallbacks(final CallbackContext callbackContext) {
    beaconServiceNotifier =
      new IBeaconServiceNotifier() {
        @Override
        public void rangingBeaconsDidFailForRegion(
          final Region region,
        ...
        @Override
        public void monitoringDidFailForRegion(
          final Region region,
          final Exception exception
        ) {
          threadPoolExecutor.execute(
            ...
            new Runnable() {
              public void run() {
                try {
                  ...
                  Log.e(
                    TAG,
                    "'startMonitoringForRegion' exception " + e.getCause()
                  );
              ...
              public void run() {
                try {
                  ...
                  callbackContext.error(
                    "didChangeAuthorizationStatus error: " + e.getMessage()
                  );
                }
              }
            }
          ...
          try {
            if (
              ...
              return new PluginResult(
                ...
                "Permission bluetooth connect not granted"
              );
            }
  ...
  private void appendToDeviceLog(
      ...
      new ILocationManagerCommand() {
        ...
        public PluginResult run() {
          ...
          try {
            ...
            Log.e(
              TAG,
              "'startMonitoringForRegion' service error: " + e.getCause()
            );
            beaconServiceNotifier.monitoringDidFailForRegion(region, e);
            return new PluginResult(PluginResult.Status.ERROR, e.getMessage());
          } catch (Exception e) {
      ...
      new ILocationManagerCommand() {
        ...
        public PluginResult run() {
          try {
            ...
            Log.e(
              TAG,
              "'stopMonitoringForRegion' service error: " + e.getCause()
            );
            return new PluginResult(PluginResult.Status.ERROR, e.getMessage());
          } catch (Exception e) {
      ...
      new ILocationManagerCommand() {
        ...
        public PluginResult run() {
          try {
            ...
            Log.e(
              TAG,
              "'startRangingBeaconsInRegion' service error: " + e.getCause()
            );
            return new PluginResult(PluginResult.Status.ERROR, e.getMessage());
          } catch (Exception e) {
            Log.e(
              TAG,
              "'startRangingBeaconsInRegion' exception " + e.getCause()
            );
            return new PluginResult(PluginResult.Status.ERROR, e.getMessage());
          }
        }
      ...
      new ILocationManagerCommand() {
        ...
        public PluginResult run() {
          try {
            ...
            Log.e(
              TAG,
              "'stopRangingBeaconsInRegion' service error: " + e.getCause()
            );
            return new PluginResult(PluginResult.Status.ERROR, e.getMessage());
          } catch (Exception e) {
            Log.e(
              TAG,
              "'stopRangingBeaconsInRegion' exception " + e.getCause()
            );
            return new PluginResult(PluginResult.Status.ERROR, e.getMessage());
          }
        }
      ...
      new ILocationManagerCommand() {
        ...
        public PluginResult run() {
          try {
            ...
            debugLog(
              "'getAuthorizationStatus' Device not supported: " + e.getMessage()
            );
            return new PluginResult(PluginResult.Status.ERROR, e.getMessage());
          } catch (Exception e) {
  ...
  private void requestStateForRegion(
    final JSONObject arguments,
    CallbackContext callbackContext
  ) {
    _handleCallSafely(
      ...
      new ILocationManagerCommand() {
        ...
        public PluginResult run() {
          try {
            ...
            //if device does not support iBeacons and error is thrown
            debugLog(
              "'isRangingAvailable' Device not supported: " + e.getMessage()
            );
            return new PluginResult(PluginResult.Status.ERROR, e.getMessage());
          } catch (Exception e) {
  ...
  private void registerDelegateCallbackId(
    ...
    _handleCallSafely(
      ...
      new ILocationManagerCommand() {
        ...
        public PluginResult run() {
          debugLog(
            "Registering delegate callback ID: " +
   ...
   */
  private void isMonitoringAvailableForClass(
    final JSONObject arguments,
    final CallbackContext callbackContext
  ) {
    _handleCallSafely(
  ...
  private void startAdvertising(
    ...
    final String uuid = arguments.has("uuid") && !arguments.isNull("uuid")
      ? arguments.getString("uuid")
      : null;
    final String major = arguments.has("major") && !arguments.isNull("major")
      ? arguments.getString("major")
      : null;
    final String minor = arguments.has("minor") && !arguments.isNull("minor")
      ? arguments.getString("minor")
      : null;

    // optinal second member in JSONArray is just a number
    final int measuredPower = args.length() > 1 ? args.getInt(1) : -55;

    if (major == null && minor != null) throw new UnsupportedOperationException(
    ...
    _handleCallSafely(
      ...
      new ILocationManagerCommand() {
        ...
        public PluginResult run() {
          ...
          final BeaconTransmitter beaconTransmitter =
            LocationManager.this.createOrGetBeaconTransmitter();
  ...
  private Region parseRegion(JSONObject json)
    ...
    if (!json.has("typeName")) throw new InvalidKeyException(
      "'typeName' is missing, cannot parse Region."
    );

    if (!json.has("identifier")) throw new InvalidKeyException(
      "'identifier' is missing, cannot parse Region."
    );
  ...
  private Region parseCircularRegion(JSONObject json)
    throws JSONException, InvalidKeyException, UnsupportedOperationException {
    if (!json.has("latitude")) throw new InvalidKeyException(
      "'latitude' is missing, cannot parse CircularRegion."
    );

    if (!json.has("longitude")) throw new InvalidKeyException(
      "'longitude' is missing, cannot parse CircularRegion."
    );

    if (!json.has("radius")) throw new InvalidKeyException(
      "'radius' is missing, cannot parse CircularRegion."
    );
  ...
  private Region parseBeaconRegion(JSONObject json)
    throws JSONException, UnsupportedOperationException {
    String identifier = json.getString("identifier");

    //For Android, uuid can be null when scanning for all beacons (I think)
    String uuid = json.has("uuid") && !json.isNull("uuid")
      ? json.getString("uuid")
      : null;
    String major = json.has("major") && !json.isNull("major")
      ? json.getString("major")
      : null;
    String minor = json.has("minor") && !json.isNull("minor")
      ? json.getString("minor")
      : null;
  ...
  private boolean hasBlueToothPermission() {
    ...
    int access = context.checkCallingOrSelfPermission(
      Manifest.permission.BLUETOOTH
    );
    int adminAccess = context.checkCallingOrSelfPermission(
      Manifest.permission.BLUETOOTH_ADMIN
    );

    return (
      (access == PackageManager.PERMISSION_GRANTED) &&
      (adminAccess == PackageManager.PERMISSION_GRANTED)
    );
  }

  //////// Async Task Handling ////////////////////////////////

  private void _handleCallSafely(
    CallbackContext callbackContext,
    final ILocationManagerCommand task
  ) {
  ...
  private void _handleExceptionOfCommand(
    CallbackContext callbackContext,
    Exception exception
  ) {
  ...
  private void _sendResultOfCommand(
    CallbackContext callbackContext,
    PluginResult pluginResult
  ) {
    //debugLog("Send result: " + pluginResult.getMessage());
    if (pluginResult.getStatus() != PluginResult.Status.OK.ordinal()) debugWarn(
      "WARNING: " + PluginResult.StatusMessages[pluginResult.getStatus()]
    );
